...
<!-- poe非官方 -->
                   <div class="block-item">
                <div class="header">
                    <span class="block-title">poe非官方</span>
                    <span style="float:right;">
                        <el-button type="text"  size="mini" v-if="!openpoeunofficalSetting" @click="openpoeunofficalSetting=true" class="text-btn">配置</el-button>
                        <el-button type="text"  size="mini" v-else @click="savePoeUnOfficalSetting" class="text-btn">保存</el-button>
                    </span>
                </div>
                <div class="body">
            <el-table
          height="calc(25vh - 20px)"
          :data="poeUnOfficalTableData"
          stripe
          border
          style="width: 100%"
         :header-row-style="{height:'30px'}"
        :header-cell-style="{padding:'0px 6px'}"
        :cell-style="{padding:openpoeunofficalSetting?'0':'6px'}"
        >
        <el-table-column prop="model" label="模型" min-width="25">
            <template slot-scope="scope">
            <span :style="{paddingLeft:openpoeunofficalSetting?'6px':'0px'}">{{scope.row.model}}</span>
            </template>
          </el-table-column>
            <el-table-column prop="count" label="次数" min-width="15">
                <template slot-scope="scope">
                    <el-input-number v-model="scope.row.count"  v-if="openpoeunofficalSetting" class="select-inputnumber"
                    size="mini" :min="1"  style="width:100%;"
                     :step="1"></el-input-number>
                    <span v-else>{{scope.row.count}}</span>
                </template>
         ...
         <el-table-column prop="cycle" label="周期" min-width="15">
                 <template slot-scope="scope">
                    <el-input-number v-model="scope.row.cycle"  v-if="openpoeunofficalSetting" class="select-inputnumber"
                    size="mini" :min="1"  style="width:100%;"
                     :step="1"></el-input-number>
                    <span v-else>{{scope.row.cycle}}</span>
                </template>
         ...
         <el-table-column prop="unit" label="计量值" min-width="15">
            ...
            <!--poe非官方 -->
                ...
                <div class="header">
                    <span class="block-title">poe非官方</span>
                    <span style="float:right;">
                        <el-button type="text"  size="mini" v-if="!openpoeunofficalpriceSetting" @click="openpoeunofficalpriceSetting=true" class="text-btn">配置</el-button>
                        <el-button type="text"  size="mini" v-else @click="savePoeUnOfficalPriceSetting" class="text-btn">保存</el-button>
                    </span>
                </div>
                <div class="body">
            <el-table
          height="calc(25vh - 20px)"
          :data="poeunofficalPriceTableData"
          stripe
          border
          style="width: 100%"
         :header-row-style="{height:'30px'}"
        :header-cell-style="{padding:'0px 6px'}"
        :cell-style="{padding:openpoeunofficalpriceSetting?'0':'6px'}"
        >
        <el-table-column prop="model" label="模型" min-width="25">
            <template slot-scope="scope">
            <span :style="{paddingLeft:openpoeunofficalpriceSetting?'6px':'0px'}">{{scope.row.model}}</span>
            </template>
          </el-table-column>
            <el-table-column prop="count" label="积分" min-width="15">
                <template slot-scope="scope">
                    <el-input-number v-model="scope.row.count"  v-if="openpoeunofficalpriceSetting" class="select-inputnumber"
                    size="mini" :min="0"  style="width:100%;"
                     :step="10"></el-input-number>
                    <span v-else>{{scope.row.count}}</span>
                </template>
         ...
         <el-table-column prop="unit" label="计量值" min-width="15">
            <template slot-scope="scope">
                <span :style="{paddingLeft:openpoeunofficalpriceSetting?'6px':'0px'}">{{scope.row.unit}}</span>
            </template>
          </el-table-column>
        </el-table>
                </div>
...
export default {
    ...
    methods: {
       ...
       async saveBingUnOfficalSetting(){
        ...
        async savePoeUnOfficalSetting(){
            this.openpoeunofficalSetting=false;
                     let data={
            roleId:this.selectRoleItem._id,
            type:'poe非官方',
            rateData:this.poeUnOfficalTableData
         }
            const { errorCode ,message} = await this.$http.postInterfaceRate(data);
            if (errorCode === "0000") {
                this.$message({
       ...
       async saveBingUnOfficalPriceSetting(){
        ...
        async saveXfyunUnOfficalPriceSetting(){
            ...
            if (errorCode === "0000") {
                ...
                async savePoeUnOfficalPriceSetting(){
            this.openpoeunofficalpriceSetting=false;
            let data={
            roleId:this.selectRoleItem._id,
            type:'poe非官方',
            priceData:this.poeunofficalPriceTableData
         }
            const { errorCode ,message} = await this.$http.postInterfacePrice(data);
            if (errorCode === "0000") {
                this.$message({
                ...
                this.getInterfacePriceList(this.selectRoleItem._id,'poe非官方');
            }else{
                this.$message({
                    message: message,
                    type: "error",
