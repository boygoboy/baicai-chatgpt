      ...
      <template slot-scope="scope">
        <el-form label-position="left" label-width="100px">
          ...
          <el-table-column prop="enablestatus" label="启用状态" min-width="15">
            <template slot-scope="scope">
                <el-switch @change="switchXfyunStatus(scope.row)" :disabled="scope.row.tokenstatus=='离线'"
...
export default {
  ...
  methods: {
        ...
        this.$refs.tokenForm.validate(valid=>{
            if(valid){
                ...
                if(this.tokenForm._id){
                   ...
                   this.$http.putXfyunList(data).then(res=>{
                    if(res.errorCode=='0000'){
                        this.$message.success('编辑xfyun资源成功！')
                        this.handleClose()
                    }else{
                     ...
                     this.$http.postXfyunList(data).then(res=>{
                    if(res.errorCode=='0000'){
    ...
    getXfyunList(){
        ...
        this.$http.getXfyunList(query).then(res=>{
            if(res.errorCode=='0000'){
                this.tableData = res.data.xfyunlist
   ...
   editKey(row){
    this.dialogVisible=true
    this.getRoleOptions()
      this.$http.getXfyunDetail(row._id).then(res=>{
          if(res.errorCode=='0000'){
            let result=res.data
            this.tokenForm={
                _id: result._id,
    ...
    deleteKey(row){
        this.$confirm('是否删除该xfyun账号？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
          this.$http.deleteXfyunList(row._id).then(res=>{
        if(res.errorCode=='0000'){
    ...
    switchXfyunStatus(row){
       const data={
              _id:row._id,
              enablestatus:row.enablestatus
       }
       this.$http.changeXfyunStatus(data).then(res=>{
        if(res.errorCode=='0000'){
