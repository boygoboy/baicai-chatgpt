...
export default {
  ...
  methods: {
        ...
        this.$refs.tokenForm.validate(valid=>{
            if(valid){
                ...
                if(this.tokenForm._id){
                   ...
                   this.$http.putHuggingList(data).then(res=>{
                    if(res.errorCode=='0000'){
                        this.$message.success('编辑hugging资源成功！')
                        this.handleClose()
                    }else{
                     ...
                     this.$http.postHuggingList(data).then(res=>{
                    if(res.errorCode=='0000'){
    ...
    getHuggingList(){
        ...
        this.$http.getHuggingList(query).then(res=>{
            if(res.errorCode=='0000'){
                this.tableData = res.data.hugginglist
   ...
   editKey(row){
    this.dialogVisible=true
    this.getRoleOptions()
      this.$http.getHuggingDetail(row._id).then(res=>{
          if(res.errorCode=='0000'){
            let result=res.data
            this.tokenForm={
                _id: result._id,
    ...
    deleteKey(row){
        this.$confirm('是否删除该hugging账号？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
          this.$http.deleteHuggingList(row._id).then(res=>{
        if(res.errorCode=='0000'){
    ...
    switchBardStatus(row){
       ...
       this.$http.changeHuggingStatus(data).then(res=>{
        if(res.errorCode=='0000'){
