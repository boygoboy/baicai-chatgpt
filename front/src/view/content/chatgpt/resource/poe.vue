      ...
      <template slot-scope="scope">
        <el-form label-position="left" label-width="100px">
          ...
          <el-table-column prop="enablestatus" label="启用状态" min-width="15">
            <template slot-scope="scope">
                <el-switch @change="switchPoeStatus(scope.row)" :disabled="scope.row.tokenstatus=='离线'"
...
export default {
  ...
  methods: {
        ...
        this.$refs.tokenForm.validate(valid=>{
            if(valid){
                ...
                if(this.tokenForm._id){
                   ...
                   this.$http.putPoeList(data).then(res=>{
                    if(res.errorCode=='0000'){
                        this.$message.success('编辑poe资源成功！')
                        this.handleClose()
                    }else{
                     ...
                     this.$http.postPoeList(data).then(res=>{
                    if(res.errorCode=='0000'){
    ...
    getPoeList(){
        ...
        this.$http.getPoeList(query).then(res=>{
            if(res.errorCode=='0000'){
                this.tableData = res.data.poelist
   ...
   editKey(row){
    this.dialogVisible=true
    this.getRoleOptions()
      this.$http.getPoeDetail(row._id).then(res=>{
          if(res.errorCode=='0000'){
            let result=res.data
            this.tokenForm={
                _id: result._id,
    ...
    deleteKey(row){
        this.$confirm('是否删除该poe账号？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
          this.$http.deletePoeList(row._id).then(res=>{
        if(res.errorCode=='0000'){
    ...
    switchPoeStatus(row){
       ...
       this.$http.changePoeStatus(data).then(res=>{
        if(res.errorCode=='0000'){
