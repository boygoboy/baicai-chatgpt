      ...
      <template slot-scope="scope">
        <el-form label-position="left" label-width="100px">
          ...
          <el-table-column prop="enablestatus" label="启用状态" min-width="15">
            <template slot-scope="scope">
                <el-switch @change="switchChatGlmStatus(scope.row)" :disabled="scope.row.tokenstatus=='离线'"
...
export default {
  ...
  methods: {
        ...
        this.$refs.tokenForm.validate(valid=>{
            if(valid){
                ...
                if(this.tokenForm._id){
                   ...
                   this.$http.putChatGlmList(data).then(res=>{
                    if(res.errorCode=='0000'){
                        this.$message.success('编辑chatglm资源成功！')
                        this.handleClose()
                    }else{
                     ...
                     this.$http.postChatGlmList(data).then(res=>{
                    if(res.errorCode=='0000'){
    ...
    getChatGlmList(){
        ...
        this.$http.getChatGlmList(query).then(res=>{
            if(res.errorCode=='0000'){
                this.tableData = res.data.chatGlmList
   ...
   editKey(row){
    this.dialogVisible=true
    this.getRoleOptions()
      this.$http.getChatglmDetail(row._id).then(res=>{
          if(res.errorCode=='0000'){
            let result=res.data
            this.tokenForm={
                _id: result._id,
    ...
    deleteKey(row){
        this.$confirm('是否删除该chatglm账号？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
          this.$http.deleteChatGlmList(row._id).then(res=>{
        if(res.errorCode=='0000'){
    ...
    switchChatGlmStatus(row){
       const data={
              _id:row._id,
              enablestatus:row.enablestatus
       }
       this.$http.changeChatGlmStatus(data).then(res=>{
        if(res.errorCode=='0000'){
